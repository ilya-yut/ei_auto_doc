Exception Indicator: Update Requests Count Monitoring (SW_01_01_SM13_CNT)


## General Overview

This Exception Indicator (EI) monitors and counts the total number of update requests that occur within a specified time period and filtering criteria. The EI acts as a high-level system health indicator that provides count-based monitoring rather than detailed request analysis, using the same data sources as SM13 (Update Records) but focusing on aggregate metrics.

The EI catches:

High-frequency update request occurrences indicating systemic processing issues or degraded system performance


Unusual spikes in update volumes that may signal underlying technical problems, processing bottlenecks, or infrastructure issues


Zero-request periods for validation that update processing is functioning normally (when monitoring for minimum thresholds)


Threshold breaches where the number of update requests exceeds acceptable operational limits


This EI provides aggregate count monitoring for system administrators who need to track overall update request volumes and patterns without analyzing individual request details.


## Problem Description

High update request counts indicate system processing instability causing:

System Performance Issues

Processing queue bottlenecks and resource exhaustion

Database update conflicts causing retry scenarios

Infrastructure problems affecting update processing servers

Process Disruptions

Update processing failures preventing data consistency
...
Data inconsistency due to incomplete update processing
...
Investigate update request patterns using SM13 transaction analysis

Check system resource levels and database performance
...
Check database locks and resource availability during peak periods

Analyze update volume patterns and error frequencies

Corrective Actions

Apply system fixes and database optimizations

Optimize update processing parameters and scheduling

Establish proactive update volume monitoring and automated resolution procedures
...
·        BACKDAYS - initial  - treated as 1 by code (today + tomorrow)

·        DURATION - initial – treated as empty by code (no time range defined)
