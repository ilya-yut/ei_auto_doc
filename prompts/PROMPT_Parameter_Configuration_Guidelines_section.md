...
**Your task:** For **every** parameter listed in the Parameters (Name (Description)) file, **in the exact order** of that file, produce a configuration guideline entry. **Group serial-number series** (e.g. KEY1, KEY2, … KEY10) into one entry with a range heading and one explanation unless business meaning differs by index. Output **only** the "Parameter Configuration Guidelines" subsection. Do not output the Parameters Reference Table, Parameter Relationships, Default Values, Practical Configuration Examples, or any other section.
...
- **Serial-number series:** When a series of parameters differ only by serial number (e.g. KEY1, KEY2, … KEY10 or KEY1_DS, KEY2_DS, … KEY10_DS), **group them into one guideline entry** with a range heading and one explanation. Use the format **PARAM_FIRST - PARAM_LAST** (Description first – Description last): and explain the whole series once (e.g. "Key field names (1–10) for the table (TABNAME) of the change document. Populated when CONVERT_KEY is used; identifies the key components."). **Exception:** If the same naming pattern (1, 2, 3, …) has **genuinely different business meaning** per index, explain them individually. Do not write separate, repetitive entries when the only difference is the serial number.
...
- Parameters with fixed options (e.g. AGGR_PERIOD, AGGR_FIELDS, DATE_REF_FLD, DURATION_UNIT, DC_IND, plus any parameter whose name or business logic suggests aggregation, e.g. agg_type, aggreg_lvl etc): main explanation + "Options" with bullet list. **Default:** short bullets (value key + brief label). Longer Options allowed when the parameter comes from the Selected parameters file (follow that file) or when extra detail is crucial for clarity.
