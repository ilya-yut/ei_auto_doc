...
Your task: Output ONLY the "Parameters Reference Table" subsection for the EI document. Use the fixed intro wording from the benchmark and build the parameters table from the input file. **Do not group** serial-number series in the table; list **every** parameter from the input as its **own row** with a single number in the first column (1, 2, 3, …). Do not output a document title, other sections, or preamble.
...
[Table:]
Add a first column with the parameter number (1, 2, 3, …). Build the parameters table from the input file: use the same column headers and column order as in the input file, with the number column as the first column. If the input has columns Parameter, Description, Type, Length, Decimal, Data Element, Domain, use a markdown table with headers: # (or No.), Parameter, Description, Type, Length, Decimal, Data Element, Domain. List **every** parameter from the input as its **own row**; first column = single number (1, 2, 3, …). Do not group serial-number series (e.g. KEY1, KEY2, … KEY10) into one row; each parameter has its own row. Include every parameter from the input; do not omit any.

Rules:
- Use the fixed intro sentence exactly as stated above. Do not reword or extend it.
- **First column (#):** Use a single number (1, 2, 3, …) for every row. List every parameter from the input as its own row; do not group serial-number series (e.g. KEY1, KEY2, … KEY10) into one row. Build the table from the input: same columns (Parameter, Description, Type, Length, Decimal, Data Element, Domain), same order. Do not omit any parameters.
...
- Tone: professional. The intro is fixed; the table is built from input with one row per parameter (no grouping).
