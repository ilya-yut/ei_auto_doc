# Exception Indicator: MD: Vendors Master General Change Log - SW_10_06_VEND_CHNG

## General Overview

This Exception Indicator (EI) monitors master data change documents in SAP to identify changes to vendor and other master data objects across configurable time windows and object dimensions. It provides visibility into change document header and detail records (CDHDR, CDPOS), enabling detection of who changed what, when, and which key fields were affected—with optional decomposition of table keys into readable key components for audit and compliance.

This EI serves as an essential control for master data governance and procurement oversight by:
- Enabling detection of vendor and master data changes that require review or approval for audit and segregation of duties
- Supporting identification of change patterns by user, transaction, and change type for accountability and root-cause analysis
- Providing visibility into change document timelines and repetitive-change windows for compliance and monitoring
- Enabling analysis of key-field-level changes (insert, update, delete) with decomposed key components for detailed audit trails
- Supporting accountability by object class, object ID (e.g. vendor), table, and field for master data stewardship

This monitoring enables organizations to detect unauthorized or anomalous master data changes, support audit and compliance reviews, and maintain visibility into vendor and related master data change history. The EI is particularly valuable for month-end controls, master data governance, and procurement oversight.

The EI uses change document tables CDHDR and CDPOS and, where applicable, vendor master (LFA1) for object descriptions.

## Problem Description

Failure to monitor master data change documents creates multiple risks across financial reporting, operational management, and compliance:

**Financial and Reporting Issues**
- Unmonitored vendor and master data changes can affect payables, bank data, and financial master data used in closing and reporting
- Late discovery of inappropriate or erroneous master data changes may require restatements or corrective journal entries
- Change document gaps or unreviewed changes can undermine audit trails required for internal and external audit
- Lack of visibility into who changed what and when can delay period-end close when reconciliations depend on change history

**Procurement and Control Risks**
- Vendor master and related changes without monitoring may enable unauthorized payment data or bank detail changes
- Unreviewed change documents by object class, table, and field can mask segregation-of-duties violations or inappropriate access
- Missing or incomplete change visibility by user and transaction limits the ability to investigate procurement and master data incidents
- Repetitive-change logic and time-window parameters, if not monitored, can leave blind spots in change coverage

**Management Visibility and Decision-Making Risks**
- Lack of consolidated change visibility delays management awareness of significant master data and vendor changes
- Unidentified change patterns (by user, table, or time) can lead to missed corrective actions and policy enforcement
- Absence of key-field-level change analysis limits the ability to prioritize reviews and focus on high-risk objects or tables

## Suggested Resolution

**Immediate Response**
- Review the change documents flagged by the EI to understand scope (object class, object ID, table, change type, and date range)
- Verify high-impact changes (e.g. vendor, bank, payment-related tables) using transaction XK02 (Change Vendor) or the relevant master data transaction to confirm legitimacy
- Check change document header and detail status and ensure no pending or duplicate reviews
- Identify business context for unusual change volume: projects, mass updates, or potential errors

**System Assessment**
- Analyze the time and date parameters used (e.g. backdays, repetitive window) to ensure the monitoring window covers the required period
- Compare current change volumes and patterns to prior periods using the same object class and table filters
- Examine distribution by user, transaction code, and change type to identify concentration or anomalies
- Assess object class and table selection to ensure critical vendor and master data objects are included
- Validate that key decomposition and repetitive-change settings align with audit and compliance requirements

**Corrective Actions**
- If unauthorized or erroneous changes are identified, correct master data via the appropriate transaction (e.g. XK02 for vendor) and document the correction
- Escalate segregation-of-duties or access issues to security and management for role and authorization review
- Update monitoring scope (object class, table, date range) to cover all relevant master data and vendor objects
- Document change review and resolution for audit trail and schedule recurring EI runs for continuous visibility
- Configure alert routing to procurement and master data owners for timely review of changes
...
| 49 | OBJECTID | Vendor | CHAR | 90 | 0 | CDOBJECTV | CHAR90 |
...
| 53 | REPET_BACKDAYS | Repetitive Backdays |  | 0 | 0 |  |  |
| 54 | TABKEY | Table Key | CHAR | 70 | 0 | CDTABKEY | CHAR70 |
| 55 | TABNAME | Table Name | CHAR | 30 | 0 | TABNAME | AS4TAB |
| 56 | TAB_DESC | Short Description | CHAR | 60 | 0 | AS4TEXT | AS4TEXT |
| 57 | TCODE | Transaction Code | CHAR | 20 | 0 | CDTCODE | TCODE |
| 58 | TEXT_CASE | Text flag | CHAR | 1 | 0 | CDXFELD | XFELD |
| 59 | UDATE | Date | DATS | 8 | 0 | CDDATUM | DATUM |
| 60 | UDATE_REPET | Repetitive Date |  | 0 | 0 |  |  |
| 61 | UNIT_NEW | Unit | UNIT | 3 | 0 | CDUNIT | CDUNIT |
| 62 | UNIT_OLD | Unit | UNIT | 3 | 0 | CDUNIT | CDUNIT |
| 63 | USERNAME | User | CHAR | 12 | 0 | CDUSERNAME | CHAR12 |
| 64 | UTIME | Time | TIMS | 6 | 0 | CDUZEIT | UZEIT |
| 65 | VALUE_NEW | New value | CHAR | 254 | 0 | CDFLDVALN | FIELDVAL |
| 66 | VALUE_OLD | Old value | CHAR | 254 | 0 | CDFLDVALO | FIELDVAL |
| 67 | WAS_PLANND | Created from Planned | CHAR | 1 | 0 | CD_PLANNED | XFLAG |

### Parameter Configuration Guidelines

**IMPORTANT:** This section provides configuration guidance for ALL 67 parameters listed in the Parameters Reference Table above.

**ACT_CHNGNO** (Document Number):

Change number of the document created by this change. Populated from change document data when applicable.

**BACKDAYS** (Backdays):

How many days to look back from today. When no date range is supplied, the EI uses today minus BACKDAYS as the start of the monitoring window for change documents. In repetitive mode (REPETITIVE = 'X'), if BACKDAYS is not set it defaults to 1 and is used for the default date window.

**CHANGENR** (Document Number):

Selection and result: change document number (CDHDR/CDPOS). Filters and identifies the change document.

**CHANGE_IND** (Appl. object change-Header Lvl):

Application object change type at header level. Used to filter CDHDR and appears in the result. Drives which change documents are returned.

**CHANGE_IND Options:**
- **U**: Update (change to existing object).
- **I**: Insert (new object).
- **E**: Delete (end-of-life / deletion).
- **D**: Other change type as defined in domain.

**CHANGE_IND_DESC** (Domain name):

Description of CHANGE_IND from domain CDCHNGIND. Populated by the EI for display; used for user-friendly change type labels.

**CHNGIND** (Change Indicator-Row lvl):

Change type at row/detail level. Filters CDPOS and appears in the result. Determines which detail records are included.

**CHNGIND Options:**
- **U**: Update (field value changed).
- **I**: Insert (new record).
- **E**: Delete (record deleted).
- **S**: Other row-level change as defined in domain.

**CHNGIND_DESC** (Domain name):

Description of CHNGIND from domain CDCHNGIND. Populated by the EI for display.

**CONVERT_KEY** ('X' - Decompose Key Field):

When set to 'X', the table key (TABKEY) is decomposed into KEY1–KEY10 (field names), KEY1_V–KEY10_V (values), and KEY1_DS–KEY10_DS (descriptions) using the table's key fields. When REPETITIVE = 'X', the code sets CONVERT_KEY = 'X' internally.

**CONVERT_KEY Options:**
- **X**: Decompose key; populate KEY1–KEY10, KEY1_V–KEY10_V, KEY1_DS–KEY10_DS.
- ** ** (initial): Do not decompose key; KEY fields remain empty.

**CUKY_NEW** (CUKY):

Currency of the new value in the change document. Represents the transaction or document currency in which the new field value is expressed. Used for display and comparison with the old value.

**CUKY_OLD** (CUKY):

Currency of the old value in the change document. Represents the transaction or document currency in which the old field value was expressed. Used for display and comparison with the new value.

**FIELD_DESC** (Short Description):

Short description of the changed field (from repository). Populated by the EI for the field name in FNAME.

**FNAME** (Field Name):

Field name in the change document detail. Used to filter CDPOS (optional) and appears in the result. Restricts which changed fields are returned.

**KEY1 - KEY10** (Field Name – Field Name):

Key field names (1–10) for the table (TABNAME) of the change document. Populated when CONVERT_KEY = 'X' (or in repetitive mode); identifies the key components of the changed record. Each KEYn holds the nth key field name of the table.

**KEY1_DS - KEY10_DS** (Short Description – Short Description):

Short descriptions for KEY1–KEY10. Populated when CONVERT_KEY is used; provides readable labels for the key fields.

**KEY1_V - KEY10_V** (Short Description – Short Description):

Key field values (1–10) for the table key. Populated when CONVERT_KEY is used; holds the actual key values from TABKEY for each key position.

**LANGU** (Language for texts):

Language for descriptions (field, table, domain). Default: system logon language. Used when retrieving texts for field, table, and domain labels.

**MANAGE_IN_UTC** ('X' - Manage in UTC):

When set to 'X', date and time for default ranges use UTC instead of system local time.

**MANAGE_IN_UTC Options:**
- **X**: Use UTC for date/time.
- ** ** (initial): Use system local date/time.

**NAME_FIRST** (First Name):

First name of the user who made the change. Populated from user master using USERNAME.

**NAME_LAST** (Last Name):

Last name of the user who made the change. Populated from user master using USERNAME.

**NAME_TEXT** (Full Name):

Full name of the user who made the change. Populated from user master using USERNAME.

**OBJECTCLAS** (Change doc. object):

Change document object class. Filters CDHDR/CDPOS. If initial, object class can be derived from TABNAME via TCDOB.

**OBJECTID** (Vendor):

Object value (e.g. vendor number for LFA1). Filters change documents by object ID. In vendor context, this is the vendor number (LIFNR).

**OBJECT_DESC** (Name):

Name of the object (e.g. vendor name). Populated by the EI when TABNAME = 'LFA1' using OBJECTID (vendor number) and a vendor description function.

**PLANCHNGNR** (Change number):

Planned change number. Populated from change document when applicable.

**REPETITIVE** ('X' - Repetitive Change):

When set to 'X', enables repetitive-change logic: REPET_BACKDAYS defines the repetitive window start, BACKDAYS defaults to 1, CONVERT_KEY is set internally, and UDATE/UDATE_REPET defaults are applied.

**REPETITIVE Options:**
- **X**: Repetitive mode; use REPET_BACKDAYS and BACKDAYS for date defaults; key decomposition is enabled.
- ** ** (initial): Standard mode; single date range from BACKDAYS.

**REPET_BACKDAYS** (Repetitive Backdays):

Used when REPETITIVE = 'X'. How many days back from today the repetitive monitoring window starts. When not set (0), the window starts from today.

**TABKEY** (Table Key):

Raw table key of the changed record. When CONVERT_KEY is used, this is decomposed into KEY1–KEY10 and KEY1_V–KEY10_V.

**TABNAME** (Table Name):

Table name of the change document. Filters CDPOS and drives object class derivation (TCDOB) when OBJECTCLAS is initial. Determines which tables' changes are returned and which key structures are used for decomposition.

**TAB_DESC** (Short Description):

Short description of the table. Populated by the EI for TABNAME.

**TCODE** (Transaction Code):

Transaction code in which the change was made. Filters CDHDR and appears in the result.

**TEXT_CASE** (Text flag):

Flag indicating whether the change is a text change. Used for display and classification.

**TEXT_CASE Options:**
- **X**: Text change.
- ** ** (initial): Not a text change.

**UDATE** (Date):

Change document date. Filters which change documents are returned by date. When no date range is supplied, the default range is derived from today minus BACKDAYS (or today minus REPET_BACKDAYS in repetitive mode).

**UDATE_REPET** (Repetitive Date):

Date range for repetitive mode. When REPETITIVE = 'X' and no repetitive date range is supplied, the EI uses the same start date as the repetitive window.

**UNIT_NEW** (Unit):

Unit of measure for the new value in the change document. Used for quantity or unit-based fields.

**UNIT_OLD** (Unit):

Unit of measure for the old value in the change document. Used for quantity or unit-based fields.

**USERNAME** (User):

User who made the change. Filters CDHDR and appears in the result. Used to populate NAME_FIRST, NAME_LAST, NAME_TEXT.

**UTIME** (Time):

Time of change. Populated from change document.

**VALUE_NEW** (New value):

New contents of the changed field. Populated from CDPOS.

**VALUE_OLD** (Old value):

Old contents of the changed field. Populated from CDPOS.

**WAS_PLANND** (Created from Planned):

Flag that the change was generated from planned changes. Used for classification and filtering.

**WAS_PLANND Options:**
- **X**: Change was generated from planned changes.
- ** ** (initial): Change was not from planned changes.

### Parameter Relationships

**Time and date parameters**
- **BACKDAYS**, **UDATE**, and **UDATE_REPET** work together to define the change document time window. When REPETITIVE is not set, the EI uses today minus BACKDAYS as the default start of the monitoring window when no date range is supplied. When REPETITIVE = 'X', the repetitive window starts at today minus REPET_BACKDAYS when no range is supplied; UDATE_REPET then uses that same start; and BACKDAYS defaults to 1 and is used for the default UDATE window (today minus BACKDAYS).
- **REPETITIVE** and **REPET_BACKDAYS**: When REPETITIVE = 'X', REPET_BACKDAYS defines how many days back from today the repetitive window starts. BACKDAYS is then used for the default UDATE window when REPETITIVE is set.

**Key decomposition parameters**
- **TABNAME**, **CONVERT_KEY**, and **KEY1 – KEY10** (and KEY1_V – KEY10_V, KEY1_DS – KEY10_DS): TABNAME filters change document detail by table. When CONVERT_KEY = 'X' (or when REPETITIVE = 'X', where the code sets CONVERT_KEY internally), the table key (TABKEY) is decomposed into key field names (KEY1–KEY10), key values (KEY1_V–KEY10_V), and key descriptions (KEY1_DS–KEY10_DS) using the table's key fields from DD03L/RPY. So TABNAME and CONVERT_KEY together determine whether and how KEY1–KEY10 are populated.

**Object class and table**
- **OBJECTCLAS** and **TABNAME**: If OBJECTCLAS is initial, the code derives object class from TABNAME via TCDOB (OBJECT from TCDOB where TABNAME in R_TABNAME). So TABNAME can drive object class selection when OBJECTCLAS is not supplied.

### Default Values

- **LANGU** — Default: system logon language; used for field and table descriptions and domain value texts.
- **BACKDAYS** — Default: `1` (when REPETITIVE = 'X' and not supplied); used to derive the default date range for UDATE. When REPETITIVE is not set, the default date range is today minus BACKDAYS when no date range is supplied.
- **REPET_BACKDAYS** — Default: initial (0); when REPETITIVE = 'X', the repetitive window starts at today minus REPET_BACKDAYS; initial (0) means the window starts from today.
- **REPETITIVE** — Default: initial (empty); standard (non-repetitive) mode when not supplied. When set to 'X', repetitive-change logic and BACKDAYS default apply.
- **CONVERT_KEY** — Default: initial (empty); key decomposition is not performed. When REPETITIVE = 'X', the code sets CONVERT_KEY = 'X' internally for repetitive processing.
- **HEADER_ONLY** — Default: initial (empty); full change document detail is returned. When set to 'X', only header records are returned and the EI exits after filling T_DATA from CDHDR.
- **MANAGE_IN_UTC** — Default: initial (empty); system time is used. When set to 'X', time is managed in UTC for date/time parameters.

**Note:** When no date range is supplied (e.g. UDATE, UDATE_REPET left initial), the EI builds default ranges from today and BACKDAYS or REPET_BACKDAYS depending on REPETITIVE.

### Practical Configuration Examples

**Use Case 1: Vendor master change log – last 30 days**
```
OBJECTCLAS = (object class for vendor, e.g. from TCDOB for LFA1)
TABNAME = LFA1
UDATE = last 30 days (e.g. GE today − 30)
BACKDAYS = 30
```
**Purpose:** Monitor vendor master (LFA1) changes in the last 30 days for audit and procurement review.

**Use Case 2: Repetitive change monitoring – weekly window**
```
REPETITIVE = X
REPET_BACKDAYS = 7
BACKDAYS = 1
...
```
**Purpose:** Run in repetitive mode with a 7-day repetitive window and default backdays for UDATE; suitable for weekly change monitoring of vendor master.

**Use Case 3: Change document header only – quick alert**
```
HEADER_ONLY = X
OBJECTCLAS = (desired object class)
OBJECTID = (optional vendor or range)
UDATE = (date range)
```
**Purpose:** Retrieve only change document headers for a fast check that changes exist; no CDPOS detail or key decomposition.

**Use Case 4: Key decomposition for audit detail**
```
CONVERT_KEY = X
TABNAME = LFA1
FNAME = (field name range if needed)
OBJECTCLAS = (vendor object class)
UDATE = (date range)
```
**Purpose:** Return change document detail with table key decomposed into KEY1–KEY10 and KEY1_V–KEY10_V for readable audit trail.

**Use Case 5: Filter by user and transaction**
```
USERNAME = (user or range)
TCODE = (transaction code or range)
CHANGE_IND = (U, I, E, D as needed)
TABNAME = LFA1
UDATE = (date range)
```
**Purpose:** Focus on changes by specific user(s) and transaction(s) for segregation-of-duties or incident review.
