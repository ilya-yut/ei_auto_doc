# Exception Indicator: SapConnect: Send Requests (SOST) - SW_01_02_SOST

## General Overview

This Exception Indicator (EI) monitors outbound SapConnect send requests in SOST and highlights delayed, failed, inconsistent, or otherwise critical communication-processing outcomes. It combines status, timing, sender/recipient, and message context to support focused operational follow-up.

This EI helps by:
- Detecting unresolved communication requests before business impact escalates
- Prioritizing backlog by elapsed duration and processing state
- Revealing status and route patterns that indicate recurring issues
- Supporting traceable incident review with message-level detail
- Providing evidence for communication-control monitoring

The function reads SOST request data, enriches status metadata, calculates duration from status timestamps, and returns exception-focused results.


## Problem Description

When SOST send requests are not monitored consistently, pending and failed communication can accumulate and reduce process reliability.

**Operational and Process Risks**
- Waiting or transit requests may remain unresolved beyond expected handling windows
- Error and inconsistency patterns can repeat without targeted remediation
- Aging backlog may grow when duration-based prioritization is absent
- Critical communications can be delayed without timely escalation

**Control and Compliance Risks**
- Weak exception traceability reduces control evidence quality
- Incomplete closure records weaken governance and audit readiness
- Inconsistent selection settings reduce comparability across review periods
- Repeated failures may persist without structured trend analysis

**Management Visibility Risks**
- Persistent communication instability may surface too late
- Resource planning is harder without clear status-aging insight
- Route/sender-specific issues may remain hidden without segmentation

### Suggested Resolution

**Immediate Response**
- Triage waiting, error, and inconsistent requests identified by the EI
- Prioritize oldest unresolved requests via duration thresholds
- Escalate high-impact failures to responsible technical/process owners
- Record retransmission and correction outcomes

**System Assessment**
- Analyze trends by status, sender, route type, and monitoring window
- Validate lookback, duration, and status-selector configuration
- Verify UTC/local time handling consistency where landscapes are distributed
- Tune max-record settings for performance and visibility balance

**Corrective Actions**
- Refine filtering/alerting logic for recurring failure signatures
- Address root causes in configuration, address data, or integration behavior
- Standardize exception closure evidence and ownership tracking
- Run recurring governance review of persistent status outliers
- Feed remediation outcomes back into monitoring configuration
...
BACKDAYS sets the implicit lookback horizon (in days) when no explicit date range is passed, so queue/backlog checks always have a baseline time window.
...
USER_FLD is a dynamic recipient/user context selector whose concrete meaning depends on implementation-specific mapping logic in this EI flow.
...
- Sender/recipient and route fields (for example **SENDER**, **RC_NAME**, **SNDART**) support targeted investigation.
- Message fields (**MSGID**, **MSGNO**, **MSGTY**, **MSGV1-4**) add diagnostic detail.

**Volume and Execution Balance:**

- **MAX_RECORDS** works with date/status filters to balance runtime and coverage.
...
- **Status fallback** - Default included states: WAIT, ERROR, INCONS, TRANSIT, OK
- **MAX_RECORDS** - 500.

### Practical Example of Parameter Configuration
**Use Case 1: Short-delay queue monitoring**
...
**Purpose:** Detect queued requests that exceed short operational response thresholds.

**Use Case 2: Weekly failed-request remediation view**

```plaintext
SND_DATE = 20260309-20260316
STAT_ERROR = X
STAT_INCONS = X
DURATION = 1
DURATION_UNIT = H
...
**Purpose:** Focus remediation on unresolved failed and inconsistent communication over the review week.

**Use Case 3: Full-day SLA escalation**
...
**Purpose:** Escalate requests that remain pending for at least one full day.
