# Exception Indicator: SD Active Quotation Status ( SW_10_01_QUOT_STAT)

## General Overview

This Exception Indicator identifies active sales quotations whose overall processing, delivery, billing, and credit-check status fields match configured criteria, returning quotation header status together with optional item detail and partner attributes when enabled.

This EI serves as an essential control for quotation management by:

- Enabling detection of quotations with overall or individual status values that require review while validity dates indicate the quotation is still active
- Supporting monitoring of processing, delivery, billing, and confirmation status on open quotation documents
- Applying quotation and contract validity date logic so only currently active documents are evaluated
- Providing visibility into organizational, customer, and partner context on each quotation
- Supporting age-based prioritization when quotations remain in scope after a chosen reference date
- Supporting optional item-level expansion when detailed line review is required

Typical use includes active quotation exception monitoring, blocked-quotation sampling, and periodic review of quotations with specific status combinations before customer follow-up or conversion to orders. Results are intended for exception workflows rather than operational quotation list reporting.

The routine reads sales quotation header data joined to overall document status, applies validity-date and date-window filters, optionally expands item detail, enriches partner and customer description data, and raises an alert when qualifying rows remain.


## Problem Description

Failure to monitor active quotation status creates multiple risks across sales, pricing, and customer engagement:

**Sales and Operations Risks**

- Quotations with blocked or incomplete overall status can delay customer response, conversion, or follow-up without structured review
- Active quotations past internal review thresholds can remain open with outdated status combinations
- Individual status fields that remain in error or pending state can hide specific control gaps on open quotes

**Operational Risks**

- Monitoring windows misaligned with quotation entry cadence can exclude recent exceptions or retain expired quotes
- Validity-date scope that is not tuned can mix expired quotations into the review queue or exclude still-active cases
- Status filters that are too broad or too narrow can hide actionable quotations or create reviewer fatigue

**Control and Audit Risks**

- Weak quotation-status monitoring reduces evidence that flagged documents were reviewed before customer commitment
- Lack of recurring exception review limits accountability for sales follow-up on stalled quotations
...
- Review flagged quotations for overall processing status, validity dates, delivery and billing status, customer, and sales organization
- Confirm with sales whether the current status and validity window are correct or require correction or follow-up action
- Prioritize high-value customers and long-aged active quotations for immediate follow-up

**System Assessment**

- Validate lookback window, reference-date field, validity-date handling, and age threshold settings against quotation review cadence
...
- Resolve status blocks or update quotations through standard SD processes where review confirms action is required
- Adjust monitoring scope after cleanup so results reflect truly exceptional status cases on active quotations
- Document review outcomes and schedule recurring runs before quotation expiry or conversion milestones
...
| 9 | BLOCK | Indicator: Document preselected for archiving | CHAR | 1 | 0 | BLOCK_VB | BLOCK_VB |
...
| 41 | DATE_REF_FLD | Date reference field |  | 0 | 0 |  |  |
...
| 58 | ITEM_DETAILS | CHAR | 1 | 0 |  | XFELD |  |
...
| 70 | MANAGE_IN_UTC | 'X' - Manage in UTC |  | 0 | 0 |  |  |
...
Stabilizes week-over-week metrics by fixing single-character indicator (DUMMY) while allowing duration thresholds to move.
...
Combines with related filters so char on ITEM_DETAILS refines which records remain for duration or state checks.
...
**Lookback window:** When no explicit date range is supplied on individual date fields, **BACKDAYS** builds a lookback window from the current day before quotations are read.

**Reference date mapping:** **DATE_REF_FLD** directs the lookback window to the created-on, changed-on, document, requested delivery, or quotation and contract validity dates. When **DATE_REF_FLD** is initial and a single document category value is supplied, the reference field defaults to valid-from for contracts or quotation valid-from for quotations.

**Quotation validity:** For quotation document categories, quotation valid-from and valid-to dates are applied so only quotations that are currently active relative to the evaluation date are retained.

**Status selection:** Overall processing, delivery, billing, rejection, confirmation, credit-check, block, and incompletion status fields filter quotations by processing, delivery, billing, and credit-check state from the sales document header status.
...
**Partner roles:** **BP1_FUNCT** / **BP1_CODE**, **BP2_FUNCT** / **BP2_CODE**, and **BP3_FUNCT** / **BP3_CODE** work together to enrich and filter business partner attributes on each quotation.

**Item detail:** When **ITEM_DETAILS** is set, matching quotation item rows are appended with material, quantity, and value fields from quotation items.
...
**Use Case 1: Incomplete overall processing on active quotations**

**Purpose:** Review active quotations with overall processing status indicating incomplete processing in one sales organization.
...
**Use Case 2: Quotation validity with delivery status focus**

**Purpose:** Monitor active quotations with delivery status indicating items not fully delivered.
...
**Use Case 3: Billing block status on quotations**

**Purpose:** Sample active quotations with overall billing block status requiring follow-up.
...
**Use Case 4: Sold-to partner with rejection status**

**Purpose:** Review active quotations for one sold-to partner with a specific overall rejection status.
...
**Use Case 5: Exactly seven full days since quotation valid-from date**

**Purpose:** Return rows whose quotation valid-from reference date is exactly 7 full days ago for weekly follow-up.
