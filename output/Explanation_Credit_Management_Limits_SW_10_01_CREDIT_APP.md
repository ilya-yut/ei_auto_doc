# Exception Indicator: Credit Management Limits ( SW_10_01_CREDIT_APP)

## General Overview

This Exception Indicator monitors change-document activity related to Credit Management Limits by showing who changed customer credit-limit and related credit-control data, when the change occurred, which transaction was used, and which fields moved from old to new values. It gives credit, sales, and compliance teams a focused review list for credit-limit maintenance follow-up.

This EI serves as an essential control for credit limit governance by:
- Enabling detection of unauthorized or unexpected changes to credit limits and related credit-control attributes
- Supporting accountability by identifying the responsible user, change timing, and before/after values for limit-related updates
- Helping credit teams prioritize review of limit increases, decreases, and related master-data changes that affect customer exposure
- Providing readable object and key context so reviewers can interpret which credit-limit record was changed
- Supporting recurring surveillance of credit-limit change activity before order release, billing, or credit committee reviews

This monitoring is useful for credit-limit change reviews, exposure-policy compliance checks, and audit sampling of high-risk limit updates. It is especially relevant where credit operations need evidence that credit-limit changes were reviewed before downstream sales and fulfillment processes rely on the updated limits.

The EI uses SAP change-document header and item data configured for Credit Management Limits review.


## Problem Description

Failure to monitor Credit Management Limits–related change activity creates risks across credit control, collections, and revenue protection:

**Credit and Collections Risks**

- Unauthorized or undocumented credit-limit changes can increase customer exposure without timely review
- Limit decreases or related credit-control updates can go unnoticed and disrupt order release or collections handling
- Undetected limit changes on credit-controlled customers can leave commercial exposure unmanaged before delivery or billing

**Operational Risks**

- Review windows that do not match credit-limit maintenance cadence can miss recent changes or retain stale history
- Scope that is too broad floods credit reviewers with unrelated change lines; scope that is too narrow can hide critical limit updates
- Header-only checks speed triage but hide field-level old and new values needed for credit-limit decision follow-up

**Control and Audit Risks**

- Weak monitoring reduces evidence that Credit Management Limits changes were reviewed on a timely basis
- Missing user and object context delays escalation of high-risk or unauthorized credit-limit updates
...
- Review flagged changes for the affected credit-limit object, changed fields, old and new values, user, transaction, and change date
- Confirm with credit management whether each limit change was authorized, documented, and aligned with credit policy
- Prioritize limit increases, large limit movements, and related changes to blocking, risk category, or credit-control attributes

**System Assessment**

- Validate object-class and table scope against the credit-limit domains that must be monitored
- Tune lookback and multi-date review windows to match how quickly credit teams must detect new limit changes
- Decide whether header-only checks or full item-level detail, including readable key context, is required for actionable credit-limit review

**Corrective Actions**

- Correct unauthorized or erroneous credit-limit values through standard credit and master-data processes where review confirms action is required
- Adjust monitoring scope and review windows after cleanup so results stay focused on true Credit Management Limits exceptions
- Document review outcomes and schedule recurring runs before credit committee meetings, order-release peaks, or close milestones
...
This table lists all configurable input parameters for the Credit Management Limits monitoring configuration. Users set values for these parameters to filter and control which credit-limit–related change-document activity is selected and returned for review.
...
| 54 | OBJECTID | Customer / credit limit object | CHAR | 90 | 0 | CDOBJECTV | CHAR90 |
