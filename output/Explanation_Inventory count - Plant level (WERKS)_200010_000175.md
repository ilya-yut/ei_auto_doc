# Exception Indicator: Inventory count - Plant level(WERKS) - SW_10_02_INV_CNT_PLN

## General Overview

This Exception Indicator (EI) monitors physical inventory count documents in Materials Management (MM) to identify inventory differences by plant, storage location, and document, across configurable time windows and aggregation levels. It provides visibility into count variances, difference amounts, and status duration for audit and operational control.

This EI serves as an essential control for inventory management and financial oversight by:
- Enabling detection of significant inventory count differences that exceed thresholds and require investigation or adjustment
- Supporting identification of plants or documents with exceptional variances for prioritization and root-cause analysis
...
The EI uses physical inventory header and item data (IKPF, ISEG) and optional valuation/currency logic; output is filtered and aggregated according to the configured parameters.


## Problem Description

Failure to monitor physical inventory count differences creates multiple risks across financial reporting, operational management, and compliance:
...
- Missing aggregation by plant or document level prevents management from focusing on high-impact exceptions
...
Physical inventory indicator for cycle counting. Restricts or displays; domain XFELD-like.

**ABCIN Options:**
- **X**: Set (cycle count indicator).
- ** ** (space or initial): Not set.

**ABS_DMBTR** (Difference amount):

Absolute difference amount. Restricts or displays; also appears in the result. Business meaning: absolute value of count difference in local currency.

**AEDAT** (Changed On):

Date the record was last changed. Restricts or displays; also appears in the result.

**AGG_LVL** (Agg.Level):

Aggregation level for grouping count data (e.g. by plant, storage location, document). Restricts or populates result grouping. The parameter controls which dimensions and level of detail the result set has: empty returns single count item records; WERKS summarizes by plant and posting date with a sum of difference amount and threshold check; IBLNR summarizes by physical inventory document and fiscal year. Choose empty for item-level audit, WERKS for plant-level exception lists, or IBLNR for document-level follow-up.
...
Material category. Restricts or displays; also appears in the result.
...
Document date. Restricts which count documents are included; when DATE_REF_FLD = BLDAT, the default range is applied. Also appears in the result.

**BSTAR** (Stock Type):

Stock type. Restricts or displays; domain XFELD-like.

**BSTAR Options:**
- **X** or domain value: Set.
- ** ** (space or initial): Not set.

**BUCHM** (Book quantity):

Book quantity. Restricts or displays; also appears in the result.

**BUCHW** (Book val. at SP):

Book value at standard price. Restricts or displays; also appears in the result. Business meaning: value of book quantity at standard price.

**BUDAT** (Posting Date):

Posting date. Restricts which count documents are included; when DATE_REF_FLD = BUDAT (default in code), the default range is applied. Also appears in the result.

**CHARG** (Batch):

Batch number. Restricts or displays; also appears in the result.

**COMP_OPERATOR** (Operator):

Comparison operator for difference or amount filters. Used with RESULT_COMP or DIFF_AMOUNT when comparing values (e.g. EQ, NE, GT, LT).

**DATE_REF_FLD** (Date Ref Field):

Name of the date field used as the reference for the default date range and for duration calculation. Determines which date (posting date, document date, count date, planned count date) is used when no explicit range is supplied and for computing how long the document has been in the system.

**DATE_REF_FLD Options:**
- **BUDAT**: Posting date (default in code).
- **BLDAT**: Document date.
...
Difference amount threshold. Restricts which count items are included by difference amount (e.g. only items with difference above or below this value). Used with COMP_OPERATOR when applicable.

**DIWZL** (Diff. value):

Difference value. Restricts or displays; also appears in the result. Business meaning: value of count difference.

**DMBTR** (Difference amount):

Difference amount in local currency. Restricts or displays; also appears in the result. Business meaning: amount of count difference.

**DSTAT** (Adjustment status):

Adjustment status. Restricts which count items are included; also appears in the result.

**DSTAT Options:**
- **A**, **B**, **C** or other domain values: Adjustment status (domain DSTAT).
- ** ** (space or initial): Not set.
...
Unit of entry. Restricts or displays; also appears in the result.

**ERFMG** (Qty in un. of entry):

Quantity in unit of entry. Restricts or displays; also appears in the result.

**EXVKW** (Sales Value):

Sales value. Restricts or displays; also appears in the result. Business meaning: sales value of quantity.

**GIDAT** (Planned count date):

Planned count date. Restricts which count documents are included; when DATE_REF_FLD = GIDAT, the default range is applied. Also appears in the result.

**GJAHR** (Fiscal Year):

Fiscal year. Restricts which count documents are included; also appears in the result.

**GRUND** (Reason for inventory diff.):

Reason for inventory difference. Restricts or displays; also appears in the result.

**IBLNR** (Phys. Inventory Doc.):

Physical inventory document number. Restricts which count documents are included; also appears in the result.

**KDAUF** (Sales Order):

Sales order number. Restricts or displays; also appears in the result when special stock or sales order stock is used.

**KDEIN** (Sales order schedule):

Sales order schedule line. Restricts or displays; also appears in the result.

**KDPOS** (Sales order item):

Sales order item. Restricts or displays; also appears in the result.

**KUNNR** (Customer):

Customer number. Restricts or displays; also appears in the result when relevant to special stock.

**KWART** (Inventory val.-only mat):

Inventory valuation–only material indicator. Restricts or displays; domain XFELD.

**KWART Options:**
- **X**: Set (inventory valuation only).
- ** ** (space or initial): Not set.

**LANGU** (Language):

Language key used when retrieving material and field descriptions. When not supplied, a default (e.g. system language) may be used.

**LGORT** (Storage Location):

Storage location. Restricts which count documents and items are included; also appears in the result.

**LIFNR** (Supplier):

Vendor number. Restricts or displays; also appears in the result when relevant to special stock (e.g. consignment).

**LSTAT** ("Delete" status):

Delete status. Restricts which count items are included; also appears in the result.

**LSTAT Options:**
- **A**, **B**, **C** or other domain values: Delete status (domain DSTAT).
- ** ** (space or initial): Not set.

**MATNR** (Material):

Material number. Restricts or displays; also appears in the result.

**MAT_DESC** (Material Description):

Material description. Populated from material master; used for display.

**MBLNR** (Material Document):

Material document number. Restricts or displays; also appears in the result.

**MEINS** (Base Unit of Measure):

Base unit of measure. Restricts or displays; also appears in the result.
...
Material document year. Restricts or displays; also appears in the result.

**NBLNR** (Recount document):

Recount document number. Restricts or displays; also appears in the result.

**PLPLA** (Distr. differences):

Distribution of differences. Restricts or displays; also appears in the result.
...
Work breakdown structure element. Restricts or displays; also appears in the result when relevant to project stock.
...
Comparison value for difference or amount filters. Used with COMP_OPERATOR when filtering or validating.

**SOBKZ** (Special Stock):

Special stock indicator (e.g. consignment, sales order). Restricts which count items are included; also appears in the result.
...
- **O**: Subcontracting.
- **Q**: Project stock.
- ** ** (space or initial): Normal stock.

**SPERR** (Posting Block):

Posting block. Restricts which count documents or items are included; also appears in the result.

**SPERR Options:**
- **X**: Set (posting blocked).
- ** ** (space or initial): Not set.

**USNAD** (Adj. posting made by):

User who made the adjustment posting. Restricts or displays; also appears in the result.

**USNAM** (Changed by(Item lvl)):

User who changed the item. Restricts or displays; also appears in the result.

**USNAM_HD** (User name):

User name (header level). Restricts or displays; also appears in the result.

**USNAZ** (Counted By):

User who performed the count. Restricts or displays; also appears in the result.

**VGART** (Trans./Event Type):

Transaction/event type (count type). Restricts which count documents are included; also appears in the result.

**VKMZL** (Diff.SalesPrs + VAT):

Difference in sales price including VAT. Restricts or displays; also appears in the result. Business meaning: value difference including VAT.

**VKNZL** (Diff.SalesPr w/o VAT):

Difference in sales price without VAT. Restricts or displays; also appears in the result. Business meaning: value difference excluding VAT.

**VKWRA** (Sales value w/o VAT):

Sales value without VAT. Restricts or displays; also appears in the result. Business meaning: sales value excluding VAT.

**VKWRT** (Sales Value inc. VAT):

Sales value including VAT. Restricts or displays; also appears in the result. Business meaning: sales value including VAT.
...
Plant. Restricts which count documents and items are included; also appears in the result.

**WRTBM** (Value book qty):

Value of book quantity. Restricts or displays; also appears in the result. Business meaning: value of book quantity.

**WRTZL** (Val. cntd qty):

Value of counted quantity. Restricts or displays; also appears in the result. Business meaning: value of counted quantity.

**XAMEI** (Alternative Unit):

Alternative unit indicator. Restricts or displays; domain XFELD.

**XAMEI Options:**
- **X**: Set (alternative unit).
- ** ** (space or initial): Not set.

**XBLNI** (Phys. Inventory Ref.):

Physical inventory reference. Restricts or displays; also appears in the result.

**XBUFI** (Freeze book invntory):

Freeze book inventory indicator. Restricts which count documents or items are included; also appears in the result.

**XBUFI Options:**
- **X**: Set (book inventory frozen).
- ** ** (space or initial): Not set.

**XDIFF** (Difference posted):

Difference posted indicator. Restricts or displays; also appears in the result.

**XDIFF Options:**
- **X**: Set (difference posted).
- ** ** (space or initial): Not set.

**XLOEK** (Item Deleted):

Item deleted indicator. Restricts or displays; also appears in the result.
...
Zero count indicator. Restricts or displays; also appears in the result.
...
Recount indicator. Restricts or displays; also appears in the result.
...
Item counted indicator. Restricts or displays; also appears in the result.
...
Material document item. Restricts or displays; also appears in the result.

**ZEILI** (Item):

Count document item. Restricts or displays; also appears in the result.

**ZLDAT** (Count date):

Count date. Restricts which count documents are included; when DATE_REF_FLD = ZLDAT, the default range is applied. Also appears in the result.

**ZSTAT** (Count status):

Count status. Restricts which count items are included; also appears in the result.

**ZSTAT Options:**
- **A**, **B**, **C** or other domain values: Count status (domain DZSTAT).
- ** ** (space or initial): Not set.
...
**Use Case 1: Recent count differences by plant**
```
BACKDAYS = 7
DATE_REF_FLD = BUDAT
AGG_LVL = WERKS
...
**Use Case 2: Single-record detail for audit**
```
BACKDAYS = 30
DATE_REF_FLD = BUDAT
AGG_LVL = 
