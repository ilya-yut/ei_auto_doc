# Exception Indicator: Servers Memory Controlliing - SW_01_20_SRV_MEM

## General Overview

This Exception Indicator monitors application server memory and swap posture using counters collected through the standard application-server and saposcol-style interfaces. It helps basis and operations teams see when free physical memory, swap consumption, or paging rates move outside agreed guardrails before paging storms or out-of-memory failures affect dialog and batch work.

This EI serves as an essential control for infrastructure and capacity management by:

- Enabling early detection of hosts where free memory or swap free percentage no longer meets internal runway targets.
- Supporting prioritization when paging-in or paging-out rates spike on specific instances or clusters.
- Providing a repeatable snapshot aligned with how administrators correlate physical usage, configured swap, and observed swap pressure.
- Helping release and month-end windows by surfacing memory stress that may precede database or application alerts.
- Giving management a concise signal that memory and swap health are reviewed with the same discipline as CPU and dialog metrics.

Typical use cases include daily capacity triage, investigation after user-reported slowness, and evidence for hardware or cloud sizing reviews.

The function relies on standard SAP server discovery and memory measurement services, then applies the configured multivalued filters to the returned metrics before alert evaluation.


## Problem Description

Failure to monitor server memory and swap utilization in a structured way creates multiple risks across application stability, database performance, and the ability to recover before paging or exhaustion events disrupt business processing.

**Performance and Availability Risks**

- Shrinking free memory or rising swap usage can slow work processes and garbage collection long before formal capacity actions complete.
- Elevated paging rates can indicate mis-sized kernels, driver issues, or competing workloads that simple averages hide.
- Swap exhaustion or misconfiguration can turn a single noisy job into a wide outage across co-located instances.

**Operational and Recovery Risks**

- Teams may react only after short dumps, database alerts, or mass user complaints, missing earlier concentration on a subset of hosts.
- Without comparable metrics across runs, administrators cannot tell seasonal noise from a new memory baseline.
- Emergency mitigation becomes subjective when no agreed monitoring lens ties physical, swap, and paging dimensions together.

**Management Visibility and Decision-Making Risks**

- Leadership lacks a compact narrative on whether memory headroom keeps pace with application growth between formal reviews.
- Procurement and cloud sizing lack objective series tied to the same server names already referenced in incident tickets.
- Post-incident reviews struggle to show that monitoring scope matched the contracted server population.

## Suggested Resolution

**Immediate Response**

- When the monitor highlights a host or server row, confirm the same condition in your standard OS or hypervisor console for that instance before opening a major incident.
- Identify whether the signal is driven by free memory percentage, absolute free kilobytes, swap free percentage, or paging rates so remediation matches the real driver.
- Check whether the spike aligns with known batch archives, large extracts, or recent kernel or parameter changes.
- Capture the metric snapshot for the infrastructure owner if change or capacity tickets are required.

**System Assessment**

- Compare current memory and swap posture to the prior week for the same host and application server slice.
- Validate whether paging counters move together with swap usage, which informs cleanup versus expansion decisions.
- Correlate with CPU and dialog metrics when interactive degradation is reported without an obvious database cause.
- Review whether collection subtype or type filters still match the intended landscape after migrations.

**Corrective Actions**

- Relieve pressure through safe housekeeping, job rescheduling, or instance scaling when sustained violations exceed internal guardrails.
- Correct swap configuration or storage paths when misalignment is confirmed.
...
Server and host selectors define which application server instances contribute rows before physical memory, free memory, and usage filters narrow the list to the posture you intend to review. Swap size, configured swap, free swap, and swap usage fields work together with swap free percentage so operators can judge both absolute headroom and relative pressure. Paging counters for pages and kilobytes per second complement utilization percentages when distinguishing thrashing from steady high use.

When several numeric thresholds are active, each dimension still applies to the same retrieved interval: a row must satisfy the server scope and every populated memory or swap filter before it influences alerting. Type and subtype fields help keep collections comparable across hardware generations when mixed landscapes are monitored under one definition.
...
**Use Case 1: Tight free-memory guard**
...
**Use Case 3: Paging spike review**
