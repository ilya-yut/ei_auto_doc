...
This Exception Indicator (EI) monitors material valuation data to identify materials where the standard price was not set or is zero (or otherwise outside a configured ratio to the moving average price). It uses material valuation from plant and valuation area to compare standard price (STPRS) and moving average price (VERPR), and flags materials whose ratio indicates a missing or incorrect standard price for management review.

This EI serves as an essential control for material costing and master data by:
- Enabling detection of materials with no or incorrect standard price that may distort inventory valuation and cost of goods sold
- Supporting identification of materials requiring standard price maintenance for month-end close and costing
- Providing visibility into price control (standard vs moving average) and valuation class by plant and material group
- Enabling analysis of materials that should have a standard price set for costing and audit
- Supporting accountability for material master and valuation data quality

This monitoring helps organizations detect costing exceptions, maintain standard prices where required, and keep inventory valuation reliable. The EI is particularly valuable for cost accounting, material master maintenance, and audit of valuation data.

The EI uses material valuation (MBEW), plant data (T001W), plant-level material (MARC), and material master (MARA) to evaluate standard vs moving average price and return materials that meet the configured criteria (e.g. standard price not set or ratio outside range).


## Problem Description

Failure to monitor materials where the standard price was not set or is incorrect creates multiple risks across financial reporting, operational management, and compliance:

**Financial and Reporting Issues**
- Materials without a standard price or with zero standard price can distort inventory valuation and cost of goods sold when standard costing is used
- Unset or obsolete standard prices may lead to misstated period-end closing and financial statements
- Unmonitored materials missing standard prices can delay month-end close when discovered late during review
- Incorrect or missing material valuation affects product costing and profitability analysis

**Operational and Control Risks**
- Materials with standard price not set may indicate master data gaps, missing price updates, or process failures in standard price maintenance
- Lack of visibility into which materials lack standard prices hinders costing policy enforcement
- Unmonitored valuation and price control patterns can mask data quality issues
- Absence of monitoring limits the ability to prioritize materials for standard price maintenance

**Management Visibility and Decision-Making Risks**
- Management may be unaware of materials with unset standard prices until audit or period-end issues arise
- Unidentified gaps delay corrective action and standard price maintenance
...
**Immediate Response**
- Review the materials flagged by the EI to confirm whether the standard price is intentionally not set (e.g. moving-average-only materials) or indicates an error or gap
- Verify high-value or high-volume materials with missing or zero standard price for correctness and policy alignment
- Check price control (standard vs moving average) and valuation class for flagged materials to ensure standard price is required where applicable
- Identify business context: planned price update pending, data entry error, or systemic maintenance gap

**System Assessment**
- Analyze the time window (e.g. last change date) and organizational scope (plants, material groups) of the results
- Compare current results to prior periods to identify recurring or worsening materials without standard price
- Examine distribution by plant, material type, and valuation class to find patterns
- Assess whether standard price maintenance process or role assignment needs change
- Validate the lookback period and date basis used for the monitoring window

**Corrective Actions**
- Set or update standard prices (e.g. CK24, MR21) where policy requires standard price and it is missing or wrong
- For materials that should remain on moving average only, ensure price control and valuation settings are correct and documented
...
- Use EI output to prioritize materials for standard price maintenance and to support costing and audit reviews
...
Indicator whether the material is costed with quantity structure. The EI uses it when selecting material valuation.
...
Material number. The EI uses it when selecting material master, plant-level material, and material valuation.
...
Material type. The EI uses it when selecting material master.
...
Ratio of standard price to moving average price (STPRS/VERPR). The EI computes it for each material and uses it to filter by the configured range; materials where standard price was not set (e.g. STPRS = 0) or ratio is outside the range can be flagged.
...
Standard price of the material in the valuation area. The EI uses it when selecting material valuation and when computing the standard-to-moving-average ratio; when STPRS is zero the material may be flagged as "standard price wasn't set."
...
**SMRATIO and STPRS/VERPR Connection:** SMRATIO is computed as STPRS/VERPR; the EI filters by the configured SMRATIO range to return materials whose standard-to-moving-average ratio (or lack of standard price) falls within that range. STPRS and VERPR are the inputs to the ratio.
...
**Standard price and ratio**

- **SMRATIO**, **STPRS**, and **VERPR** work together: the EI computes SMRATIO as STPRS/VERPR for each material and filters by the configured SMRATIO range. STPRS (standard price) and VERPR (moving average price) are read from material valuation; when STPRS is zero or ratio is outside the range, the material can be flagged as "standard price wasn't set" or requiring review.
...
**Use Case 1: Materials where standard price wasn't set – last 100 days**
```
BACKDAYS = 100
SMRATIO = 0 - 0.01
WERKS = 1010
```
**Purpose:** Find materials in plant 1010 with very low or zero standard-to-moving-average ratio (e.g. standard price not set or zero) over the last 100 days.
...
SMRATIO = 0 - 0.5
```
**Purpose:** Focus on materials in material group 001 with standard price control (VPRSV = S) and ratio below 0.5 (e.g. standard price missing or too low) over the last 60 days.
...
MTART = FERT
```
**Purpose:** Review finished goods (MTART = FERT) in plants 1010 and 1020 with valuation class 3000 where standard price is effectively not set (ratio 0–0.01) over the last 90 days.

**Use Case 4: Narrow ratio band and language**
```
BACKDAYS = 30
SMRATIO = 0 - 0.1
LANGU = E
...
WERKS = 1010
```
**Purpose:** Tight band (0–0.1) for a specific material and plant over 30 days to flag missing or very low standard price, with English descriptions.
...
MATKL = 002
```
**Purpose:** External procurement materials (BESKZ = E) with special procurement 10 in plant 1010 and material group 002 where standard price is not set or ratio &lt; 0.5, last 45 days.
