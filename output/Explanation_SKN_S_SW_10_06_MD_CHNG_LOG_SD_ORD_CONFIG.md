# Exception Indicator: Sales Order with Product Configuration Change (SW_10_06_MD_CHNG_LOG – SD)

## General Overview

This Exception Indicator monitors change-document activity for sales orders where product configuration–related attributes were changed. It shows who changed the sales order or configuration-relevant data, when the change occurred, which transaction was used, and which fields moved from old to new values. It gives sales, order-management, and compliance teams a focused review list for configuration-driven order changes that can affect pricing, ATP, manufacturing, or delivery commitments.

This EI serves as an essential control for SD sales-order configuration governance by:
- Enabling detection of unauthorized or unexpected changes to product configuration on open or recently processed sales orders
- Supporting accountability by identifying the responsible user, change timing, and before/after field values for configuration-related updates
- Helping sales and fulfillment teams prioritize review of sensitive configuration changes that can alter product composition, options, or commercial terms
- Providing readable object and key context so reviewers can interpret which sales order, item, or configuration object was changed
- Supporting recurring surveillance of sales-order configuration change activity before confirmation, production, delivery, or billing steps rely on the updated configuration

This monitoring is useful for sales-order change reviews, configurable-product governance, and audit sampling of high-risk SD configuration updates. It is especially relevant where order operations need evidence that product-configuration changes were reviewed before downstream SD and logistics processes consume them.

The EI uses SAP change-document header and item data configured for Sales Order with Product Configuration Change review.


## Problem Description

Failure to monitor sales-order product configuration change activity creates risks across order integrity, fulfillment accuracy, and audit compliance:

**Sales and Fulfillment Risks**

- Unauthorized or undocumented configuration changes can alter product options, BOM-relevant choices, or commercial attributes without timely review
- Sensitive configuration fields can change without clear visibility of who made the update and what values moved
- Undetected configuration changes on open sales orders can leave promised dates, pricing, or manufacturing instructions misaligned before delivery or billing

**Operational Risks**

- Review windows that do not match order-change cadence can miss recent configuration updates or retain stale history
- Scope that is too broad floods SD reviewers with unrelated change lines; scope that is too narrow can hide critical configuration updates
- Header-only checks speed triage but hide field-level old and new values needed for configuration follow-up

**Control and Audit Risks**

- Weak monitoring reduces evidence that sales-order product-configuration changes were reviewed on a timely basis
- Missing user and object context delays escalation of high-risk or unauthorized configuration updates
- Lack of recurring exception review limits accountability between sales, master data, and order-fulfillment owners

## Suggested Resolution

**Immediate Response**

- Review flagged changes for the affected sales order or configuration object, changed fields, old and new values, user, transaction, and change date
- Confirm with sales order management whether each configuration change was authorized, documented, and aligned with order-change policy
- Prioritize changes that affect product options, configuration characteristics, pricing-relevant attributes, or delivery-critical order data

**System Assessment**

- Validate object-class and table scope against the SD sales-order and product-configuration domains that must be monitored
- Tune lookback and multi-date review windows to match how quickly sales teams must detect new configuration changes
- Decide whether header-only checks or full item-level detail, including readable key context, is required for actionable SD review

**Corrective Actions**

- Correct unauthorized or erroneous configuration-related values through standard sales-order and configuration processes where review confirms action is required
- Adjust monitoring scope and review windows after cleanup so results stay focused on true Sales Order with Product Configuration Change exceptions
- Document review outcomes and schedule recurring runs before order confirmation peaks, production handoff, delivery cutoff, or close milestones
