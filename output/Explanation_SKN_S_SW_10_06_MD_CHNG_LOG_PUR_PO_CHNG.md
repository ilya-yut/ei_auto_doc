# Exception Indicator: Purchase Order Changes (SW_10_06_MD_CHNG_LOG – PUR)

## General Overview

This Exception Indicator monitors change-document activity related to purchase order changes by showing who changed purchasing documents or purchase-relevant attributes, when the change occurred, which transaction was used, and which fields moved from old to new values. It gives procurement, purchasing, and compliance teams a focused review list for PO change follow-up that can affect commitments, supplier terms, quantities, prices, or delivery schedules.

This EI serves as an essential control for PUR purchase-order change governance by:
- Enabling detection of unauthorized or unexpected changes that affect purchase order content, commercial terms, or fulfillment commitments
- Supporting accountability by identifying the responsible user, change timing, and before/after field values for purchasing updates
- Helping procurement teams prioritize review of sensitive attribute changes that can alter spend exposure, supplier obligations, or goods-receipt readiness
- Providing readable object and key context so reviewers can interpret which purchase order, item, or purchasing record was changed
- Supporting recurring surveillance of purchase-order change activity before goods receipt, invoice verification, or period-close reviews rely on the updated PO

This monitoring is useful for PO change reviews, purchasing compliance checks, and audit sampling of high-risk procurement updates. It is especially relevant where purchasing operations need evidence that PO changes were reviewed before downstream MM and AP processes consume them.

The EI uses SAP change-document header and item data configured for Purchase Order Changes review.


## Problem Description

Failure to monitor purchase-order change activity creates risks across procurement control, spend integrity, and audit compliance:

**Procurement and Supply Risks**

- Unauthorized or undocumented PO changes can alter quantities, prices, delivery dates, or supplier terms without timely review
- Sensitive purchasing attributes can change without clear visibility of who made the update and what values moved
- Undetected change activity on open purchase orders can leave commitments, GR/IR readiness, or supplier communication unmanaged before receipt or invoice posting

**Operational Risks**

- Review windows that do not match purchasing change cadence can miss recent PO updates or retain stale history
- Scope that is too broad floods procurement reviewers with unrelated change lines; scope that is too narrow can hide critical PO updates
- Header-only checks speed triage but hide field-level old and new values needed for purchasing decision follow-up

**Control and Audit Risks**

- Weak monitoring reduces evidence that purchase-order changes were reviewed on a timely basis
- Missing user and object context delays escalation of high-risk or unauthorized purchasing updates
- Lack of recurring exception review limits accountability between buyers, category owners, and master-data or AP stakeholders

## Suggested Resolution

**Immediate Response**

- Review flagged changes for the affected purchase order or purchasing object, changed fields, old and new values, user, transaction, and change date
- Confirm with purchasing whether each change was authorized, documented, and aligned with PO-change policy
- Prioritize changes that affect net price, quantity, delivery schedule, vendor, payment terms, or other commitment-critical attributes

**System Assessment**

- Validate object-class and table scope against the purchasing domains that must be monitored for PO changes
- Tune lookback and multi-date review windows to match how quickly procurement teams must detect new changes
- Decide whether header-only checks or full item-level detail, including readable key context, is required for actionable PUR review

**Corrective Actions**

- Correct unauthorized or erroneous purchase-order values through standard ME22N/purchasing processes where review confirms action is required
- Adjust monitoring scope and review windows after cleanup so results stay focused on true Purchase Order Changes exceptions
- Document review outcomes and schedule recurring runs before GR peaks, invoice cycles, or close milestones
