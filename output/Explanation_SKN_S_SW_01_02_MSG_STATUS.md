# Exception Indicator: Message Status (NAST) - SW_01_02_MSG_STATUS

## General Overview

This Exception Indicator reviews SAP output determination records in the condition technique context so teams can see which messages are active, scheduled, or in exceptional states for selected application areas and business objects.

This EI helps by:
- Giving support and functional teams a bounded list of message rows that match configurable filters instead of scanning broad condition output lists
- Supporting operational reviews when output status, processing state, or timing deviates from what downstream communication expects
- Highlighting concentration of issues by application, output type, language, partner role, or derived presentation attributes when volumes grow unevenly
- Providing repeatable evidence for periodic controls around customer-facing or logistics-related messaging flows
- Helping auditors tie monitoring results to the same business objects they validate in standard output determination and message handling

Typical use includes daily checks after configuration changes, troubleshooting batches that did not issue expected outputs, and validating cleanup after incidents. Teams still use standard condition output and message transactions when a single object needs immediate correction.

The routine reads message-related selection data, applies multivalued filters, evaluates presentation-oriented attributes, and applies duration-style aging where configured before surfacing rows for review.


## Problem Description

Failure to monitor output determination and message lifecycle health creates multiple risks across operational delivery, customer communication, and internal control evidence.

**Operational and Delivery Risks**
- Delayed or missing outputs can remain hidden until downstream processes, billing, or shipping already consumed inconsistent information
- Large undifferentiated extracts make it difficult to see which object families or communication types drive most of the noise
- Intermittent states are hard to prioritize when reviewers cannot quickly separate active, suppressed, or failed-style populations

**Control and Compliance Risks**
- Weak recurring review weakens evidence that messaging behavior was supervised during sensitive close or migration windows
- Inconsistent handling of language, partner, or channel dimensions increases the chance that the wrong audience receives sensitive content
- Without a structured slice of records, remediation work is harder to document and trace to root cause

**Management Visibility and Decision-Making Risks**
- Leadership lacks a compact view of where messaging workload concentrates across applications and object types
- Trend comparison week over week is unreliable when monitoring scope drifts or is recreated ad hoc each time
- Cross-team triage slows when operations and application support cannot share the same filtered view of problematic rows

## Suggested Resolution

**Immediate Response**
- Review each flagged line in business terms: object type, communication intent, and current processing posture before changing master data
- Open the standard message and output determination tools that fit the object family to validate what the business sees today
- Capture when the finding ties to regulated or customer-visible communications so follow-up is prioritized appropriately
- Segment the population by the strongest business dimensions first so effort goes to the highest-impact rows
- Confirm whether the situation is transient configuration drift versus persistent master-data or rule gaps

**System Assessment**
- Compare current results to the prior monitoring cycle after releases, transports, or batch jobs that touch output determination
- Look for concentration by application, output category, partner role, and language to see whether the issue is localized or systemic
- Validate whether recent changes to user or transaction context explain spikes in unusual rows
- Examine whether presentation-oriented attributes cluster in a way that suggests one integration path is misconfigured
- Check whether aging-style signals align with known batch windows or only appear off-cycle

**Corrective Actions**
- Correct master data, access, or output determination content according to SAP guidance, then re-run monitoring to confirm the population is back within tolerance
- Adjust monitoring scope after root cause so benign patterns are excluded without hiding genuine risk
- Document remediation and control-owner sign-off when messaging issues affected materially sensitive processes
- Schedule recurring runs during critical business windows and archive exports for audit when evidence is required
- Route repeat findings into a defect or change process when configuration or custom logic must be updated centrally
