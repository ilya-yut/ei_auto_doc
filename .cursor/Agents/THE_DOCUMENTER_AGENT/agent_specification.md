...
## ABAP Function (EI) Documentation from User Sample

**Mode:** Document an ABAP function (Exception Indicator) from five user inputs. Output is **one file** that matches a **user-provided sample** structure exactly.

**Inputs (implementation folders):**
- **input/** (per-function; add/replace for each next function): Function code (`Code_*.txt`), structure (`Structure_*.xlsx`), parameter list ("Parameters" sheet of `Available fields_*.xlsx`). Or **CHUNKS/** + **DOCUMENTER_INSTRUCTIONS.md** when THE_CHUNKER was run first (long functions, e.g. 2k+ lines).
- **reference/** (shared; same for any function): **Sample document** (format authority, e.g. `sample_format.md`), **Parameter reference file** (e.g. `_Parameters.docx`).
- **output/** — Generated documentation (e.g. `Explanation_*.md` or `.docx`).

*(The **reference files** folder is for planning/verification/debug only — temporary when verifying .cursor; not used in implementation.)*

**Optional pre-step:** When the function code is long (e.g. 2k+ lines), run **THE_CHUNKER** with `/chunk:ei_function [path_to_Code_*.txt]`. It produces CHUNKS/ + DOCUMENTER_INSTRUCTIONS.md; then use CHUNKS/ as the "code" input for this workflow. See `.cursor/Agents/THE_CHUNKER_AGENT/agent_specification.md`.

**Workflow:**
1. **Parse sample** → Extract section titles, levels, order; table shapes. This is the format spec.
2. **Load content** → Code file, structure xlsx, parameter list.
3. **Parameter text** → For each parameter from the list, look up name in parameter reference file. If found → use that text **verbatim**. If not found → generate from code/domain. No line numbers or custom implementation details in narrative.
4. **Fill sections** → Map content to sections using the sample's order. Use EI domain logic for General Overview / Problem Description / Suggested Resolution only where the sample includes such sections.
5. **Emit** → One file that strictly follows the parsed format. Same section count and order; same table structures.
6. **Verify** → Read generated file; confirm structure matches sample and all parameters covered.

**Authority:** See `.cursor/rules/abap-function-doc-format-authority.md` and `.cursor/rules/document-authority.md` ("For ABAP Function / EI Documentation"). Plugin: `abap_function_doc_plugin.yaml`.

**Example:**
```
User: "Document this ABAP function using input/ and reference/"

Agent: Parses reference/sample_format.md → loads input/Code_*.txt, input/Structure_*.xlsx, input/Available fields_*.xlsx (Parameters sheet) → uses reference/_Parameters.docx verbatim for matching params → fills sections from sample order → emits one file to output/ matching sample structure.
```
