...
#### For ABAP Function / EI Documentation

When the task is documenting an ABAP function (Exception Indicator) from user inputs:

```
1. User-provided SAMPLE document       → Format authority (sections, order, tables) — from reference/
2. Parameter reference file (_Parameters.docx or equivalent) → Text authority for parameters — from reference/
3. Code file, structure file, parameter list → Content sources (fill sections) — from input/
```

**Implementation folders:** **input/** (per-function: code, structure, Available fields); **reference/** (shared: sample_format, _Parameters); **output/** (generated docs). The *reference files* folder is for planning/verification/debug only, not implementation.

**Do not** use llm_handover.md / WORKFLOW.md / CLAUDE.md as format or content authority for this task. Use `.cursor/rules/abap-function-doc-format-authority.md` for format and parameter behavior.
