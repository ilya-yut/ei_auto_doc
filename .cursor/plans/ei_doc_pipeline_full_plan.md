...
| File | Purpose |
|------|--------|
| [scripts/pipeline/pipeline.py](scripts/pipeline/pipeline.py) | Main script: `verify()`, `prepare`, `assemble`. Run from repo root: `python scripts/pipeline/pipeline.py prepare [--skip-verify]` or `assemble`. |
| [scripts/pipeline/run_pipeline.bat](scripts/pipeline/run_pipeline.bat) | Runs prepare → pause (with instructions) → assemble. |
| [scripts/pipeline/CURSOR_INSTRUCTION.txt](scripts/pipeline/CURSOR_INSTRUCTION.txt) | Exact wording to send in Cursor after prepare (generate 7 sections, write to run/01_response.md … 07_response.md). |
...
Copy-paste this to Cursor after Step 1 has finished:
...
(This text must also be saved in `scripts/pipeline/CURSOR_INSTRUCTION.txt` so you can open it from the project.)
...
The file `scripts/pipeline/CURSOR_INSTRUCTION.txt` must contain the **exact wording of Step 2** above (the block you copy-paste to generate the 7 sections). That way you can open the file in the project and send its content to Cursor instead of retyping it.
