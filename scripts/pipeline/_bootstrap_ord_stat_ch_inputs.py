"""Bootstrap input files for SW_10_01_ORD_STAT_CH (S/4 Sales order general)."""
...
INPUT = ROOT / "input"
OLD = INPUT / "old"
STEM = "SKN_S_SW_10_01_ORD_STAT_CHK"
STRUCT_NAME = "/SKN/S_SW_10_01_ORD_STAT_CHK"
INDICATOR_ID = "SW_10_01_ORD_STAT_CH"
INDICATOR_NAME = "S/4 Sales order general"
...
def main() -> None:
    ...
    for pat in [
        "*ORD_STAT.xlsx",
        "Code_SKN_S_SW_10_01_ORD_STAT.txt",
        "Available fields_SKN_S_SW_10_01_ORD_STAT.xlsx",
        "Metadata _SKN_S_SW_10_01_ORD_STAT.xlsx",
    ...
    src_code = next(INPUT.glob("Code_*ORD_STAT_CH*"), None) or next(
        INPUT.glob("Code_*Sales order general*"), None
    )
    src_avail = next(INPUT.glob("Available*ORD_STAT_CH*"), None) or next(
        INPUT.glob("Available*Sales order general*"), None
    )
    if not src_code or not src_avail:
        raise SystemExit("missing code or available-fields source for ORD_STAT_CH")
    ...
    extras = {
        ...
        "REF_FIELD1": ["Reference field 1", "CHAR", "30", "0", "FIELDNAME", "FDNAME"],
        "REF_FIELD2": ["Reference field 2", "CHAR", "30", "0", "FIELDNAME", "FDNAME"],
        "COMP_OPERATOR": ["Comparison operator", "CHAR", "2", "0", "BUCC_OPERATOR", ""],
        "WAERK_FR": ["Foreign currency", "CUKY", "5", "0", "WAERK", "WAERS"],
        "INC_NOT_CONV_CURR": ["Include not converted currency", "CHAR", "1", "0", "", "BOOLE"],
    }
