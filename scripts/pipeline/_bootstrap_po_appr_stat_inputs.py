"""Bootstrap input files for SW_10_03_PO_APPR_STA (PUR PO Approval State)."""
...
INPUT = ROOT / "input"
OLD = INPUT / "old"
STEM = "SKN_S_SW_10_03_PO_APPR_STAT"
STRUCT_NAME = "/SKN/S_SW_10_03_PO_APPR_STAT"
INDICATOR_ID = "SW_10_03_PO_APPR_STA"
INDICATOR_NAME = "PUR PO Approval State"


def main() -> None:
    OLD.mkdir(exist_ok=True)

    for pat in ["*PO_1_TIME*", "*OPEN_PR*", "*AG_ALV*"]:
    ...
    src_code = next(INPUT.glob("Code_*PO_APPR*"), None)
    src_avail = next(INPUT.glob("Available*PO_APPR*"), None)
    if not src_code or not src_avail:
        raise SystemExit("missing code or available-fields source for PO_APPR_STAT")
