"""Bootstrap input files for SW_10_03_PR_APPR_WAT (PR Waiting for Approval)."""
...
INPUT = ROOT / "input"
OLD = INPUT / "old"
STEM = "SKN_S_SW_10_03_PR_APPR_WAIT"
STRUCT_NAME = "/SKN/S_SW_10_03_PR_APPR_WAIT"
INDICATOR_ID = "SW_10_03_PR_APPR_WAT"
INDICATOR_NAME = "PR Waiting for Approval"


def main() -> None:
    OLD.mkdir(exist_ok=True)

    for pat in ["*PO_SH_LEAD*", "*PO_LEADTIME*"]:
    ...
    src_code = next(INPUT.glob("Code_*PR_APPR_WAT*"), None) or next(
        INPUT.glob("Code_*PR Waiting*"), None
    )
    src_avail = next(INPUT.glob("Available*PR_APPR_WAT*"), None) or next(
        INPUT.glob("Available*PR Waiting*"), None
    )
    if not src_code or not src_avail:
        raise SystemExit("missing code or available-fields source for PR_APPR_WAIT")
