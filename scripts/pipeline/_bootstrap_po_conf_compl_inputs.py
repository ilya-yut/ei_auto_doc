"""Bootstrap input files for SW_10_03_PO_CONF_COM (Waiting for Complete PO Confirmation)."""
...
INPUT = ROOT / "input"
OLD = INPUT / "old"
STEM = "SKN_S_SW_10_03_PO_CONF_COMPL"
STRUCT_NAME = "/SKN/S_SW_10_03_PO_CONF_COMPL"
INDICATOR_ID = "SW_10_03_PO_CONF_COM"
INDICATOR_NAME = "Waiting for Complete PO Confirmation"


def main() -> None:
    OLD.mkdir(exist_ok=True)

    for pat in ["*PO_APPR_BY*", "*PO_APPR_WAIT*", "*PO_APPR_STAT*"]:
    ...
    src_code = next(INPUT.glob("Code_*PO_CONF*"), None) or next(INPUT.glob("Code_*Confirmation*"), None)
    src_avail = next(INPUT.glob("Available*PO_CONF*"), None) or next(INPUT.glob("Available*Confirmation*"), None)
    if not src_code or not src_avail:
        raise SystemExit("missing code or available-fields source for PO_CONF_COMPL")
