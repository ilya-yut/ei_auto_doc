"""Bootstrap input files for SW_10_01_ORD_ST_CH_E (ECC Sales order general)."""
...
INPUT = ROOT / "input"
OLD = INPUT / "old"
STEM = "SKN_S_SW_10_01_ORD_STAT_CHK_ECC"
STRUCT_NAME = "/SKN/S_SW_10_01_ORD_STAT_CHK"
INDICATOR_ID = "SW_10_01_ORD_ST_CH_E"
INDICATOR_NAME = "ECC Sales order general"
...
def main() -> None:
    ...
    src_code = next(INPUT.glob("Code_*ORD_ST_CH*"), None) or next(
        INPUT.glob("Code_*ECC Sales*"), None
    ) or next(OLD.glob("Code_*ORD_ST_CH*"), None) or next(
        OLD.glob("Code_*ECC Sales*"), None
    )
    src_avail = next(INPUT.glob("Available*ORD_ST_CH*"), None) or next(
        INPUT.glob("Available*ECC Sales*"), None
    ) or next(OLD.glob("Available*ORD_ST_CH*"), None) or next(
        OLD.glob("Available*ECC Sales*"), None
    )
    if not src_code or not src_avail:
        raise SystemExit("missing code or available-fields source for ORD_ST_CH_E")
