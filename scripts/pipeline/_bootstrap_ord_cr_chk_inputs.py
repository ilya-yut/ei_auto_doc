"""Bootstrap input files for SW_10_01_ORD_CR_CHK (SD Order Credit Checks Status)."""
...
INDICATOR_ID = "SW_10_01_ORD_CR_CHK"
INDICATOR_NAME = "SD Order Credit Checks Status"
...
def main() -> None:
    ...
    for pat in ["*MIN_ORD*", "*Minimum order*"]:
    ...
    src_code = next(INPUT.glob("Code_*ORD_CR_CHK*"), None) or next(
        INPUT.glob("Code_*Credit Checks*"), None
    )
    src_avail = next(INPUT.glob("Available*ORD_CR_CHK*"), None) or next(
        INPUT.glob("Available*Credit Checks*"), None
    )
    if not src_code or not src_avail:
        raise SystemExit("missing code or available-fields source for ORD_CR_CHK")
    ...
    for r in ws.iter_rows(min_row=3, values_only=True):
        ...
        if key == "LANG":
            ...
            oldp[key] = list(r)
        if key not in {p.upper() for p in ordered}:
            ordered.append(key if key != "LANGU" else "LANGU")
    for fp in FRAMEWORK_PARAMS:
        if fp.upper() not in {x.upper() for x in ordered}:
            ordered.insert(0, fp)
    # ensure framework params first in stable order
    ...
    for p in FRAMEWORK_PARAMS + tail:
        ...
        if u in seen:
            continue
        seen.add(u)
        code_params.append(p if p != "LANGU" or "LANGU" in seen else p)
    if ws.max_row >= 3:
    ...
    print(f"ready {len(code_params)} params, {len(fields)} fields, metadata at {meta_path.name}")
