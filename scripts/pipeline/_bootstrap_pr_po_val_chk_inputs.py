"""Bootstrap input files for SW_10_03_PR_PO_VAL_C (PREQ vs PO value check)."""
...
INPUT = ROOT / "input"
OLD = INPUT / "old"
STEM = "SKN_S_SW_10_03_PR_PO_VAL_CHK"
STRUCT_NAME = "/SKN/S_SW_10_03_PR_PO_VAL_CHK"
INDICATOR_ID = "SW_10_03_PR_PO_VAL_C"
INDICATOR_NAME = "PREQ vs PO value check"


def main() -> None:
    OLD.mkdir(exist_ok=True)

    for pat in ["*PR_APPR_WAIT*", "*PR Waiting*"]:
    ...
    src_code = next(INPUT.glob("Code_*PR_PO_VAL*"), None) or next(
        INPUT.glob("Code_*PREQ vs PO*"), None
    )
    src_avail = next(INPUT.glob("Available*PR_PO_VAL*"), None) or next(
        INPUT.glob("Available*PREQ vs PO*"), None
    )
    if not src_code or not src_avail:
        raise SystemExit("missing code or available-fields source for PR_PO_VAL_CHK")
