"""Bootstrap input files for SW_10_06_LAND_COMPAR (LAND_CODE_COMP)."""
...
INPUT = ROOT / "input"
OLD = INPUT / "old"
STEM = "SKN_S_SW_10_06_LAND_CODE_COMP"
STRUCT_NAME = "/SKN/S_SW_10_06_LAND_CODE_COMP"
INDICATOR_ID = "SW_10_06_LAND_COMPAR"
INDICATOR_NAME = "Company code country differs vendor's country"
...
SRC_CODE_GLOB = "Code_*LAND_COMPAR*"
SRC_AVAIL_GLOB = "Available*LAND_COMPAR*"
...
def main() -> None:
    ...
    for pat in ["*PO_CONTR_VALID*", "Metadata _SKN_S_SW_10_02_PO_CONTR_VALID*"]:
    ...
    if not src_code or not src_avail:
        raise SystemExit("Missing code or available-fields source for LAND_COMPAR")
    ...
    extras = {
        ...
        "LFA1_LAND1": ["Vendor country", "CHAR", "3", "0", "LAND1", "LAND1"],
    }
    for i, fld in enumerate(CODE_PARAMS, start=3):
        row = oldp.get(fld.upper())
        if row is None and fld == "LFA1_LAND1":
            row = oldp.get("LAND1")
        if row is None:
            row = extras.get(fld)
        ws.cell(i, 1, fld)
