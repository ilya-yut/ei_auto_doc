"""Bootstrap input files for SW_10_01_BD_PT_CHK (Billing Data Payment Terms check)."""
...
INPUT = ROOT / "input"
OLD = INPUT / "old"
STEM = "SKN_S_SW_10_01_SD_PT_CHK"
STRUCT_NAME = "/SKN/S_SW_10_01_SD_PT_CHK"
INDICATOR_ID = "SW_10_01_BD_PT_CHK"
INDICATOR_NAME = "Billing Data Payment Terms check"


def main() -> None:
    OLD.mkdir(exist_ok=True)

    for pat in ["*ARG_200005*", "*AR_000073*", "*Sales order*"]:
    ...
    src_code = next(INPUT.glob("Code_*BD_PT_CHK*"), None) or next(
        INPUT.glob("Code_*Billing Data*"), None
    )
    src_avail = next(INPUT.glob("Available*BD_PT_CHK*"), None) or next(
        INPUT.glob("Available*Billing Data*"), None
    )
    if not src_code or not src_avail:
        raise SystemExit("missing code or available-fields source for BD_PT_CHK")
