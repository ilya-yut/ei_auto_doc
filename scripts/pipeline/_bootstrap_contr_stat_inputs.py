"""Bootstrap input files for SW_10_01_CONTR_STAT (SD Active Contract Status)."""
...
INDICATOR_ID = "SW_10_01_CONTR_STAT"
INDICATOR_NAME = "SD Active Contract Status"


def main() -> None:
    OLD.mkdir(exist_ok=True)

    for pat in ["*SD_PT_CHK*", "*BD_PT*", "*Billing Data*"]:
    ...
    src_code = next(INPUT.glob("Code_*CONTR_STAT*"), None) or next(
        INPUT.glob("Code_*Contract Status*"), None
    )
    src_avail = next(INPUT.glob("Available*CONTR_STAT*"), None) or next(
        INPUT.glob("Available*Contract Status*"), None
    )
    if not src_code or not src_avail:
        raise SystemExit("missing code or available-fields source for CONTR_STAT")
