"""Bootstrap input files for SW_10_01_ORD_VAT_CHK (SD Order - VAT Check)."""
...
INPUT = ROOT / "input"
OLD = INPUT / "old"
STEM = "SKN_S_SW_10_01_ORD_VAT_CHK"
STRUCT_NAME = "/SKN/S_SW_10_01_ORD_VAT_CHK"
INDICATOR_ID = "SW_10_01_ORD_VAT_CHK"
INDICATOR_NAME = "SD Order - VAT Check"
...
def main() -> None:
    ...
    for pat in [
        ...
        "*ORD_ST_CH*",
        "Code_ECC*",
        "Available fields_ECC*",
        "Metadata *ECC*",
        "Code_SD Order*",
        "Available fields_SD Order*",
    ]:
    ...
    src_code = next(INPUT.glob("Code_*ORD_VAT*"), None) or next(
        INPUT.glob("Code_*VAT Check*"), None
    ) or next(OLD.glob("Code_*ORD_VAT*"), None) or next(
        OLD.glob("Code_*VAT Check*"), None
    )
    src_avail = next(INPUT.glob("Available*ORD_VAT*"), None) or next(
        INPUT.glob("Available*VAT Check*"), None
    ) or next(OLD.glob("Available*ORD_VAT*"), None) or next(
        OLD.glob("Available*VAT Check*"), None
    )
    if not src_code or not src_avail:
        raise SystemExit("missing code or available-fields source for ORD_VAT_CHK")
    ...
    extras = {
        ...
        "VAT_CONV": ["Conventional VAT rate", "DEC", "5", "2", "/SKN/E_VAT", "/SKN/E_VAT"],
        "ZERO_VAT_EXPLICITLY": ["Require zero-VAT definition", "CHAR", "1", "0", "", "XFELD"],
        "TAX_DIFF": ["Tax difference", "CURR", "13", "2", "MWSBP", "WERTV7"],
    }
