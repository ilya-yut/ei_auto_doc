...
Run from repo root: python scripts/pipeline/pipeline.py prepare [--skip-verify] [--yes] | assemble
"""
...
def prepare(skip_verify: bool = False, assume_yes: bool = False, update_manifest_only: bool = False) -> None:
    ...
    print("When the 7 response files are in scripts/pipeline/run/, run: python scripts/pipeline/pipeline.py assemble")
...
def assemble() -> None:
    ...
    if missing:
        print("Missing response files:", ", ".join(missing), "- Run Cursor step first.", file=sys.stderr)
        sys.exit(1)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="EI Doc Pipeline: prepare | assemble")
    parser.add_argument("mode", choices=["prepare", "assemble"], help="prepare (verify + write prompts) or assemble (build .md + .docx)")
