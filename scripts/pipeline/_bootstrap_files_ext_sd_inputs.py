"""Bootstrap input files for TD_10_01_FILES_EXT (Files analyzing with extension - SD)."""
...
INPUT = ROOT / "input"
OLD = INPUT / "old"
STEM = "ZSWS_TAD_10_01_FILES_EXT"
STRUCT_NAME = "ZSWS_TAD_10_01_FILES_EXT"
INDICATOR_ID = "TD_10_01_FILES_EXT"
INDICATOR_NAME = "Files analizing with extention (SD)"
...
def main() -> None:
    ...
    src_code = next(INPUT.glob("Code_*FILES_EXT*"), None) or next(
        INPUT.glob("Code_*Files*"), None
    ) or next(OLD.glob("Code_*FILES_EXT*"), None) or next(
        OLD.glob("Code_*Files analizing*"), None
    )
    src_avail = next(INPUT.glob("Available*FILES_EXT*"), None) or next(
        INPUT.glob("Available*Files analizing*"), None
    ) or next(OLD.glob("Available*Files analizing*"), None)
    if not src_code or not src_avail:
        raise SystemExit("missing code or available-fields source for FILES_EXT")
    ...
    extras = {
        ...
        "DATE_SEL": ["Take date from monitor", "CHAR", "1", "0", "", "XFELD"],
