"""Bootstrap input files for SW_10_03_OPEN_PR_DET (Released PO waiting for GR)."""
...
INPUT = ROOT / "input"
OLD = INPUT / "old"
STEM = "SKN_S_SW_10_03_OPEN_PR_DET"
STRUCT_NAME = "/SKN/S_SW_10_03_OPEN_PR_DET"
INDICATOR_ID = "SW_10_03_OPEN_PR_DET"
INDICATOR_NAME = "Released PO waiting for GR"


def main() -> None:
    OLD.mkdir(exist_ok=True)

    src_code = next(INPUT.glob("Code_*OPEN_PR_DET*"), None)
    src_avail = next(INPUT.glob("Available*OPEN_PR_DET*"), None)
    if not src_code or not src_avail:
        raise SystemExit("missing code or available-fields source for OPEN_PR_DET")
    ...
    if src_code.resolve() != code_canon.resolve():
        if src_code.parent == INPUT:
            src_code.unlink()
