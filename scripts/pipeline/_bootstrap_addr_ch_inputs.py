"""Bootstrap input files for SW_10_01_ADDR_CH (Manually changed addresses in Sales document)."""
...
INPUT = ROOT / "input"
OLD = INPUT / "old"
STEM = "SKN_S_SW_10_01_ADDR_CH"
STRUCT_NAME = "/SKN/S_SW_10_01_ADDR_CH"
INDICATOR_ID = "SW_10_01_ADDR_CH"
INDICATOR_NAME = "Manually changed addresses in Sales document"


def main() -> None:
    OLD.mkdir(exist_ok=True)

    for pat in ["*PR_PO_VAL*", "*PREQ vs PO*"]:
    ...
    src_code = next(INPUT.glob("Code_*ADDR_CH*"), None) or next(
        INPUT.glob("Code_*changed addresses*"), None
    )
    src_avail = next(INPUT.glob("Available*ADDR_CH*"), None) or next(
        INPUT.glob("Available*changed addresses*"), None
    )
    if not src_code or not src_avail:
        raise SystemExit("missing code or available-fields source for ADDR_CH")
