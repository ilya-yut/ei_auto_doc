"""Bootstrap input files for SW_10_01_ORD_DELAY (SD Delayed Orders)."""
...
INDICATOR_ID = "SW_10_01_ORD_DELAY"
INDICATOR_NAME = "SD Delayed Orders"
...
def main() -> None:
    ...
    for pat in ["*ORD_CR_CHK*", "*Credit Checks*", "Available fields_SKN_S_SW_10_01_ORD_STAT.xlsx", "Metadata _SKN_S_SW_10_01_ORD_STAT.xlsx", "Code_SKN_S_SW_10_01_ORD_STAT.txt"]:
    ...
    src_code = next(INPUT.glob("Code_*ORD_DELAY*"), None) or next(
        INPUT.glob("Code_*Delayed Orders*"), None
    )
    src_avail = next(INPUT.glob("Available*ORD_DELAY*"), None) or next(
        INPUT.glob("Available*Delayed Orders*"), None
    )
    if not src_code or not src_avail:
        raise SystemExit("missing code or available-fields source for ORD_DELAY")
    ...
    for r in ws.iter_rows(min_row=3, values_only=True):
        ...
        if key not in {p.upper() for p in ordered}:
            ordered.append("LANGU" if key == "LANGU" else str(r[0]).strip())
