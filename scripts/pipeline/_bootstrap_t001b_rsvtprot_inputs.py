"""Bootstrap input files for SW_000000_V_T001B (Opening and Closing FI periods / ALV RSVTPROT)."""
...
INPUT = ROOT / "input"
OLD = INPUT / "old"
STEM = "SKN_S_AG_ALV_RSVTPROT"
STRUCT_NAME = "/SKN/S_AG_ALV_RSVTPROT"
INDICATOR_ID = "SW_000000_V_T001B"
INDICATOR_NAME = "Opening and Closing FI periods"


def main() -> None:
    OLD.mkdir(exist_ok=True)

    for pat in ["*OPEN_PR_DET*", "*OPEN_PO_DET*"]:
    ...
    src_code = next(INPUT.glob("Code_*T001B*"), None) or next(INPUT.glob("Code_*RSVTPROT*"), None)
    src_avail = next(INPUT.glob("Available*T001B*"), None) or next(INPUT.glob("Available*RSVTPROT*"), None)
    if not src_code or not src_avail:
        raise SystemExit("missing code or available-fields source for T001B / RSVTPROT")
