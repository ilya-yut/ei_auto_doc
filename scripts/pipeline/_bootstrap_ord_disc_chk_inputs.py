"""Bootstrap input files for SW_10_01_ORD_DISC_CH (SD Order Pricing Conditions Discount Check)."""
...
INPUT = ROOT / "input"
OLD = INPUT / "old"
STEM = "SKN_S_SW_10_01_ORD_DISC_CHK"
STRUCT_NAME = "/SKN/S_SW_10_01_ORD_DISC_CHK"
INDICATOR_ID = "SW_10_01_ORD_DISC_CH"
INDICATOR_NAME = "SD Order- Pricing Conditions (Discount)Check"
...
def main() -> None:
    ...
    for pat in ["*ORD_DELAY*", "*Delayed Orders*", "*ORD_STAT*", "Code_SKN_S_SW_10_01_ORD_STAT.txt", "Available fields_SKN_S_SW_10_01_ORD_STAT.xlsx"]:
    ...
    src_code = next(INPUT.glob("Code_*ORD_DISC*"), None) or next(
        INPUT.glob("Code_*Pricing Conditions*"), None
    )
    src_avail = next(INPUT.glob("Available*ORD_DISC*"), None) or next(
        INPUT.glob("Available*Pricing Conditions*"), None
    )
    if not src_code or not src_avail:
        raise SystemExit("missing code or available-fields source for ORD_DISC_CHK")
    ...
    if "DATUM" not in seen:
        code_params.insert(len(FRAMEWORK_PARAMS), "DATUM")
    if ws.max_row >= 3:
    ...
    extras = {
        ...
        "CUST_FILTER_FM": ["Customer filter function module", "CHAR", "30", "0", "/SKN/E_SW_RL_FM", ""],
    }
