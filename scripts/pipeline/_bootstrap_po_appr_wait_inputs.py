"""Bootstrap input files for SW_10_03_PO_APPR_WAT (PUR PO Waiting for Approval)."""
...
INPUT = ROOT / "input"
OLD = INPUT / "old"
STEM = "SKN_S_SW_10_03_PO_APPR_WAIT"
STRUCT_NAME = "/SKN/S_SW_10_03_PO_APPR_WAIT"
INDICATOR_ID = "SW_10_03_PO_APPR_WAT"
INDICATOR_NAME = "PUR PO Waiting for Approval"


def main() -> None:
    OLD.mkdir(exist_ok=True)

    for pat in ["*PO_APPR_STAT*", "*PO_1_TIME*", "*OPEN_PR*"]:
    ...
    src_code = next(INPUT.glob("Code_*PO_APPR_WAT*"), None) or next(INPUT.glob("Code_*Waiting*"), None)
    src_avail = next(INPUT.glob("Available*PO_APPR_WAT*"), None) or next(INPUT.glob("Available*Waiting*"), None)
    if not src_code or not src_avail:
        raise SystemExit("missing code or available-fields source for PO_APPR_WAIT")
