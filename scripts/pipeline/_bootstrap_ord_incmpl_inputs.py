"""Bootstrap input files for SW_10_01_SD_INCP_IND (Incomplete Sales Documents - Status Indication)."""
...
INPUT = ROOT / "input"
OLD = INPUT / "old"
STEM = "SKN_S_SW_10_01_ORD_INCMPL"
STRUCT_NAME = "/SKN/S_SW_10_01_ORD_INCMPL"
INDICATOR_ID = "SW_10_01_SD_INCP_IND"
INDICATOR_NAME = "Incomplete Sales Documents - Status Indication"
...
def main() -> None:
    ...
    src_code = next(INPUT.glob("Code_*INCP*"), None) or next(
        INPUT.glob("Code_*INCMPL*"), None
    ) or next(INPUT.glob("Code_*Incomplete*"), None
    ) or next(OLD.glob("Code_*Incomplete*"), None)
    src_avail = next(INPUT.glob("Available*INCP*"), None) or next(
        INPUT.glob("Available*Incomplete*"), None
    ) or next(OLD.glob("Available*Incomplete*"), None)
    if not src_code or not src_avail:
        raise SystemExit("missing code or available-fields source for SD_INCP_IND")
    ...
    seen: set[str] = set()
    for p in FRAMEWORK_PARAMS + ["DATUM", "TRVOG", "STATG"] + tail:
        u = p.upper()
        if u in seen:
            continue
        seen.add(u)
    ...
    extras = {
        ...
        "TRVOG": ["Transaction group", "CHAR", "1", "0", "TRVOG", "TRVOG"],
        "STATG": ["Status group", "CHAR", "2", "0", "STATG", "STATG"],
    }
