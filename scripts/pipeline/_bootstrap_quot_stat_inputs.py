"""Bootstrap input files for SW_10_01_QUOT_STAT (SD Active Quotation Status)."""
...
INPUT = ROOT / "input"
OLD = INPUT / "old"
STEM = "SKN_S_SW_10_01_QUOT_STAT"
STRUCT_NAME = "/SKN/S_SW_10_01_ORD_STAT"
INDICATOR_ID = "SW_10_01_QUOT_STAT"
INDICATOR_NAME = "SD Active Quotation Status"
...
def main() -> None:
    ...
    for pat in [
        "*OUTDT*",
        "*ORD_STAT.xlsx",
        "Structure_SKN_S_SW_10_01_ORD_STAT.xlsx",
        "Code_SD Active*",
        "Available fields_SD Active*",
        "Metadata *QUOT*",
    ]:
    ...
    src_code = next(INPUT.glob("Code_*QUOT*"), None) or next(
        INPUT.glob("Code_*Quotation*"), None
    ) or next(OLD.glob("Code_*QUOT*"), None) or next(
        OLD.glob("Code_*Quotation*"), None
    )
    src_avail = next(INPUT.glob("Available*QUOT*"), None) or next(
        INPUT.glob("Available*Quotation*"), None
    ) or next(OLD.glob("Available*QUOT*"), None) or next(
        OLD.glob("Available*Quotation*"), None
    )
    if not src_code or not src_avail:
        raise SystemExit("missing code or available-fields source for QUOT_STAT")
