"""Bootstrap input files for SW_10_03_PO_LEADTIME (Purchase order line lead time)."""
...
INPUT = ROOT / "input"
OLD = INPUT / "old"
STEM = "SKN_S_SW_10_03_PO_SH_LEAD_TIM"
STRUCT_NAME = "/SKN/S_SW_10_03_PO_SH_LEAD_TIM"
INDICATOR_ID = "SW_10_03_PO_LEADTIME"
INDICATOR_NAME = "Purchase order line lead time"


def main() -> None:
    OLD.mkdir(exist_ok=True)

    for pat in ["*PO_CONF_COMPL*", "*PO_CONF_COM*"]:
    ...
    src_code = next(INPUT.glob("Code_*PO_LEADTIME*"), None) or next(
        INPUT.glob("Code_*lead time*"), None
    )
    src_avail = next(INPUT.glob("Available*PO_LEADTIME*"), None) or next(
        INPUT.glob("Available*lead time*"), None
    )
    if not src_code or not src_avail:
        raise SystemExit("missing code or available-fields source for PO_SH_LEAD_TIM")
