"""Bootstrap input files for SW_10_03_PO_1_TIME_D (PUR One-time vendor check detail)."""
...
INPUT = ROOT / "input"
OLD = INPUT / "old"
STEM = "SKN_S_SW_10_03_PO_1_TIME_DET"
STRUCT_NAME = "/SKN/S_SW_10_03_PO_1_TIME_DET"
INDICATOR_ID = "SW_10_03_PO_1_TIME_D"
INDICATOR_NAME = "PUR. One-time vendor check  (detail)"


def main() -> None:
    OLD.mkdir(exist_ok=True)

    for pat in ["*AG_ALV_RSVTPROT*", "*T001B*"]:
    ...
    src_code = next(INPUT.glob("Code_*PO_1_TIME*"), None)
    src_avail = next(INPUT.glob("Available*PO_1_TIME*"), None)
    if not src_code or not src_avail:
        raise SystemExit("missing code or available-fields source for PO_1_TIME_DET")
