"""
Bootstrap Structure / Available fields / Metadata for SKN_S_SW_01_20_USER_STATE
from Code_User Actions Control_SW_01_20_USR_MULTI.txt (User actions control).
Run from repo root: python scripts/pipeline/_bootstrap_usr_multi_user_state.py
"""
...
INPUT = ROOT / "input"
STEM = "SKN_S_SW_01_20_USER_STATE"
STRUCT_NAME = "/SKN/S_SW_01_20_USER_STATE"
INDICATOR_ID = "SW_01_20_USR_MULTI"
INDICATOR_NAME = "User actions control"
SRC = INPUT / "Code_User Actions Control_SW_01_20_USR_MULTI.txt"
CODE_CANON = INPUT / f"Code_{STEM}.txt"


def _add_multiline_block(lines: list[str], start_idx: int, names: set[str]) -> int:
    """Parse DATA_MULTY / SELECT_MULTY style blocks with continuation lines. Returns next line index."""
    ln = lines[start_idx]
    ...
    while j < len(lines):
        s = lines[j]
        if re.match(r"^\s*(DATA_|SELECT_|LV_)", s):
            break
...
def _parse_params_from_code(text: str) -> list[str]:
    ...
    while i < len(lines):
        ...
        while i < len(lines):
            s = lines[i]
            st = s.strip()
            if not st or st.startswith('"---') or re.match(r"^\s*DATA_SINGLE:\s*SW_DEST", s):
                break
            # Continuation lines are deeply indented; avoid ABAP keywords like ENDIF.
            if len(s) - len(s.lstrip()) < 10:
                break
            mm = re.match(r"^\s*(\w+)\s*,", s) or re.match(r"^\s*(\w+)\s*\.\s*$", s)
            if mm and mm.group(1).upper() not in ("ENDIF", "ENDLOOP", "ENDCASE", "ENDFUNCTION"):
                names.add(mm.group(1))
            if "DATE_REF_FLD" in s and st.endswith("."):
                i += 1
                break
            i += 1
        continue
    if re.search(r"FIELDNM\s*=\s*'BACKDAYS'", text):
        names.add("BACKDAYS")
...
def _guess_type(field: str) -> tuple[str, str, str, str]:
    ...
    if u == "DURATION":
        return "INT4", "10", "0", u
    if u in ("MANAGE_IN_UTC", "NO_DATE_RESTRICTION", "VALID_USERS_ONLY", "LANGU", "DURATION_UNIT"):
        return "CHAR", "1", "0", u
    if u == "DATE_REF_FLD":
